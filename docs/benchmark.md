...
| what is being drawn | vs flutter_markdown_plus | vs markdown_widget |
|---|---|---|
| **Links** | **4.5x faster** | **7.2x faster** |
| **Bullet list** | **3.1x faster** | **4.7x faster** |
| **Plain prose** | **2.4x faster** | **2.8x faster** |
| **Headings** | **1.5x faster** | **7.1x faster** |
| **Mixed answer** | 1.1x slower | **1.5x faster** |
| **Table** (small) | 1.7x slower | **1.6x faster** |
| **Table** (40 rows) | **1.1x faster** | — |
| Code block | 2.9x slower | 2.5x slower |

Both packages were run three times in one sitting, and each ratio is computed
...
headings, lists — against both. The only cold-mount losses are code blocks
...
harness floor**, the plain widget in 3.4 ms above it, and the other two in
79 ms and 92 ms above it.
...
| **Streaming**, plain `GptMarkdown` | The whole document sits in one `Column`. Measured per chunk: 2 segments rebuild, 2 re-lay-out — the cache is doing its job — but every block is still walked and painted. | Partly. Collapsing the semantics of a reply while it is still arriving cut this roughly in half (see below). For the rest, use `SliverGptMarkdown`. |
...
objects to **274** for 123 cells (fmp 503, so we are the leaner of the two) and cut the per-cell measurement from ~36 us to ~20 us. A 40-row
...
cache returned 277 of 279 blocks unchanged and only 2 blocks re-laid-out, so
almost none of the cost was our own code. It was **semantics**: every block already on
...
| feature | us | fmp | markdown_widget |
|---|---|---|---|
| **LaTeX / maths** | YES, inline and block | NO | NO |
| **Built-in syntax highlighting** | YES, ~190 languages | PART — you write it | YES, via `flutter_highlight` |
| **Streaming flag** | YES | NO | NO |
| **Incremental re-render** | YES | NO | NO |
| **Character reveal animation** | YES | NO | NO |
| **Block entrance animation** | PART — widget blocks only | NO | NO |
| **Sliver / lazy rendering** | PART — real sliver, lazy per block | PART — `ListView`, parses all first | PART — `ListView`, parses all first |
| **Task list checkboxes** | YES, optionally tappable | PART — renders, not tappable | PART — renders, not tappable |
| **Images, size from alt text** | YES | PART — no alt sizing | PART — no alt sizing |
| **Custom inline syntax** | YES — patterns + directives | YES — full parser access | YES — node generators |
| **Bidi placeholder fix** | YES | NO | NO |
| **Autolinks** | YES — GFM + angle + custom schemes | YES | YES |
...
| feature | us | fmp | markdown_widget |
|---|---|---|---|
| **Per-chunk streaming cost** | **falls to ~1.0 ms** with the sliver; flat ~3-4 ms without | grows, no limit | grows, no limit |
| **Default text direction** | LTR until you pass it | — | — |
| **Footnotes** | NO | **YES** | NO |
| **Any-HTML-tag builders** | NO — fixed hooks | **YES — any tag** | **YES — any node** |
...
| feature | note |
|---|---|
| Tables + column alignment | all three |
| Lists, nesting, `start` number | all three |
| Blockquotes, nested | all three |
| Strikethrough | all three |
| Horizontal rules | all three |
| Link tap callback | all three; none launch URLs for you |
| Text selection | all three, by different routes |
| System text scaling | us and fmp |
...
streaming reply use `SliverGptMarkdown`** — its per-chunk cost falls as the
reply grows, where both of the others climb without limit.
...
smallest of the three. Not for chat — no streaming flag, no incremental render,
